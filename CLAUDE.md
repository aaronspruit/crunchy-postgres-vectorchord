...
These three rules are not optional. They apply to every change.
...
There is no `changelog:` label set here. CI labels a pull request `postgres`, `vectorchord` or `build` from the diff itself, and Dependabot labels its own `dependencies`. Rule 2 still holds: the pull request body is where a breaking change is described.
